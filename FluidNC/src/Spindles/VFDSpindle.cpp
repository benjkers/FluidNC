...
namespace Spindles {
    ...
    void VFDSpindle::group(Configuration::HandlerBase& handler) {
        ...
        handler.item("poll_ms", _poll_ms, 100, 20000);  // was min 250; lowered so load-sensing protocols (e.g. Cumark adaptive feed) can poll faster. Mind your baud rate -- each poll is a full Modbus round trip.
